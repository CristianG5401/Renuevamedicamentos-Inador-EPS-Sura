...
export function resolveConfig(overrides?: CliOverrides): ValidatedConfig {
  const resolved = {
    ...
    epsChatId: overrides?.epsChatId ?? process.env.EPS_CHAT_ID,
    idNumber: overrides?.idNumber ?? process.env.ID_NUMBER,
    idType: overrides?.idType ?? process.env.ID_TYPE,
    nothingToRenewAlertMessage: process.env.NOTHING_TO_RENEW_ALERT_MESSAGE,
    successAlertMessage: process.env.SUCCESS_ALERT_MESSAGE,
    techAlertChatId: process.env.TECH_ALERT_CHAT_ID,
    userToAlertChatId: process.env.USER_TO_ALERT_CHAT_ID,
  };
